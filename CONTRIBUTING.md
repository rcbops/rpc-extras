...
## Commits, Pull Requests and Release Note Guidelines

### Commits

Please avoid the following in commits:
...
2. Generate a new release note, and the YAML file will be located under `/releasenotes/notes`. 

   `$ reno new <version>`
3. Edit the YAML file. The text uses RST formatting.

Release notes must be written with the following guidelines in mind:
...
* A pull request (PRs) should ideally contain a single commit
...
* The PR description should also reference to the original issue.
...
* Contains a single commit, which itself has a good commit message describing the changes involved in the patch. On a rare occasion, multiple related commits in the same PR could be considered.
* Includes a YAML release note file which complies with the release notes guidelines described above.
* Does not overreach - each commit should be self contained enough to only address the issue at hand (see above).
...
* There must be at least 2 members of the rcbops engineering team who have reviewed the PR and given it the thumbsup/+1 (using the review guidelines above).
